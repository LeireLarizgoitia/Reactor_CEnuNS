...
chisqndf = fcn_np(m.values) / (130-len(m.params)) #en(centre)
print('chi2/ndf  min: ' , chisqndf)

txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/MueN/MHVE_Fef_MueN_mparams90cl.txt", "w")
content = str(m.params)
txt_file.write("".join(content) + "\n")
txt_file.close()

txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/MueN/MHVE_Fef_MueN_chisq90cl.txt", "w")
for i in range(0,len(m.values)):
    content = str(m.values[i])
    txt_file.write("".join(content) + "\n")
txt_file.write('chi2ndf'+' '+str(chisqndf))
txt_file.write(' '+'chi2'+' '+str(chisq_ML))

txt_file.close()

'Minuit_ML'

centre_mu, events_mu= fnc_events_MHVE_Fef([1.],amu2_ML)
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/MueN/MHVE_Fef_MueN_fit90cl.txt", "w")
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/MueN/MHVE_Fef_MueN_events90cl.txt", "w")
for aa in range(0,len(E_ion)):
    a_obs = E_ion[aa]
    e_obs = events_mu[aa]
    content = str(a_obs)+' '+ str(e_obs)
