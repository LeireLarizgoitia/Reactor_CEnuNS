...
'mu'
def cross_section_muN(T, Enu, mu_muB2): #mu_muB = mu / muB
    Q2= 2 *Enu**2 * M * T *1e-6 /(Enu**2 - Enu*T*1e-3) #MeV ^2
...
def cross_section_mue(Te,Enu, mu_muB2): # T = Ei
...
def cross_section_Ze(Te,Enu, gz2 ,mz2):
...
def differential_events_flux_muN(T, mu2=0.):
    ...
    iint = []
    for i in range (0,nsteps):
        iint.append(cross_section_muN(T,EE[i],mu2) * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))
...
def differential_events_flux_mue(T,mu2=0.):
    ...
    iint = []
    for i in range (0,nsteps):
        iint.append( cross_section_mue(T,EE[i],mu2)  * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))
...
def differential_events_flux_SMmuN(T, mu2=0.):
    ...
    iint = []
    for i in range (0,nsteps):
        iint.append((cross_section_SM_N(T,EE[i])+cross_section_muN(T,EE[i],mu2)) * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))
...
def differential_events_flux_SMmue(T,mu2=0.):
    ...
    iint = []
    for i in range (0,nsteps):
        iint.append((cross_section_SM_e(T,EE[i]) + cross_section_mue(T,EE[i],mu2))  * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))

'Scalar'
def differential_events_flux_sN(T, g2=0 ,m2=0):
    nsteps = 100
    Emin = 1/2 * (T + np.sqrt(T**2 + 2*T*M)) * 1e-3 #MeV
    EE = np.linspace(Emin,Enu_max,num=nsteps, endpoint=True)
    iint = []
    for i in range (0,nsteps):
        iint.append(cross_section_sN(T,EE[i],g2,m2) * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))


def differential_events_flux_se(T,g2=0 ,m2=0):
    nsteps = 1000
    Emin = 1/2* (T + np.sqrt(T**2 + 2*T*(me*1e3))) * 1e-3 #MeV
    EE = np.linspace(Emin,Enu_max,num=nsteps, endpoint=True)
    iint = []
    for i in range (0,nsteps):
        EE[i] = Emin + step*i
        iint.append(cross_section_se(T,EE[i], g2 ,m2)  * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))

'SM + S'

def differential_events_flux_SMsN(T,g2=0 ,m2=0):
    nsteps = 100
    Emin = 1/2 * (T + np.sqrt(T**2 + 2*T*M)) * 1e-3 #MeV
    EE = np.linspace(Emin,Enu_max,num=nsteps, endpoint=True)
    iint = []
    for i in range (0,nsteps):
        iint.append((cross_section_SM_N(T,EE[i]) + cross_section_sN(T,EE[i], g2,m2)) * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))

def differential_events_flux_SMse(T, g2=0 ,m2=0):
    nsteps = 1000
    Emin = 1/2* (T + np.sqrt(T**2 + 2*T*(me*1e3))) * 1e-3 #MeV
    EE = np.linspace(Emin,Enu_max,num=nsteps, endpoint=True)
    iint = []
    for i in range (0,nsteps):
        iint.append((cross_section_SM_e(T,EE[i]) + cross_section_se(T,EE[i],g2,m2)) * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))


def differential_events_flux_ZN(T, g2=0 ,m2=0):
    nsteps = 1000
    Emin = 1/2 * (T + np.sqrt(T**2 + 2*T*M)) * 1e-3 #MeV
    EE = np.linspace(Emin,Enu_max,num=nsteps, endpoint=True)
    iint = []
    for i in range (0,nsteps):
        iint.append(cross_section_ZN(T,EE[i],g2,m2) * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))

def differential_events_flux_Ze(T,g2=0 ,m2=0):
    nsteps = 1000
    Emin = 1/2* (T + np.sqrt(T**2 + 2*T*(me*1e3))) * 1e-3 #MeV
    EE = np.linspace(Emin,Enu_max,num=nsteps, endpoint=True)
    iint = []
    for i in range (0,nsteps):
        iint.append(cross_section_Ze(T,EE[i],g2,m2) * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))

'SM + Z'

def differential_events_flux_SMZN(T,g2=0 ,m2=0):
    nsteps = 1000
    Emin = 1/2 * (T + np.sqrt(T**2 + 2*T*M)) * 1e-3 #MeV
    EE = np.linspace(Emin,Enu_max,num=nsteps, endpoint=True)
    iint = []
    for i in range (0,nsteps):
        iint.append((cross_section_SM_N(T,EE[i]) + cross_section_ZN(T,EE[i], g2,m2)) * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))


def differential_events_flux_SMZe(T, g2=0 ,m2=0):
    nsteps = 1000
    Emin = 1/2* (T + np.sqrt(T**2 + 2*T*(me*1e3))) * 1e-3 #MeV
    EE = np.linspace(Emin,Enu_max,num=nsteps, endpoint=True)
    iint = []
    for i in range (0,nsteps):
        iint.append((cross_section_SM_e(T,EE[i]) + cross_section_Ze(T,EE[i],g2,m2) ) * flux_total(EE[i]))
    return (integrate.simpson(iint,EE))
...
def fnc_events_MHVE(pp=0,ind=0, g2=0. ,m2=0., parsys=[1.]):
    Enr = []
    Ei = []
    #dNdx_N = []
    #dNdx_e=[]
    ...
    if pp==0:
        ...
        for tnr in Enr:
            ...
            #dNdx.append(normalization *(differential_events_flux_muN(tnr, g2)))
            #dNdx.append(normalization *(differential_events_flux_ZN(tnr, g2,m2)))
            'SM + BSM'
            dNdx.append(normalization *(differential_events_flux_SMmuN(tnr, g2)))
            #dNdx.append(normalization *(differential_events_flux_SMsN(tnr, g2,m2)))
            #dNdx.append(normalization *(differential_events_flux_SMZN(tnr, g2,m2)))
    elif pp==1:
        ...
        for x in Ei:
            ...
            #dNdx.append(normalization *(differential_events_flux_mue(x, g2)))
            #dNdx.append(normalization *(differential_events_flux_Ze(x, g2,m2)))
            'SM + BSM'
            dNdx.append(normalization *(differential_events_flux_SMmue(x, g2)))
            #dNdx.append(normalization *(differential_events_flux_SMse(x, g2,m2)))
            #dNdx.append(normalization *(differential_events_flux_SMZe(x, g2,m2)))
    else:
...
def fcn_np(par):
    ...
    centre, events = fnc_events_MHVE(pp, ind, par_mu_muB2)
