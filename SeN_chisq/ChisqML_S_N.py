...
def fnc_events_MHVE(pp=0,ind=0, gg=0. ,mm=0., parsys=[1.]):
    ...
    if pp==0:
        ...
        for tnr in Enr:
            ...
            dNdx.append(normalization *(differential_events_flux_sN(tnr, gg,mm)))
        ...
        for x in Ei:
            ...
            dNdx.append(normalization *(differential_events_flux_se(x, gg,mm)))
    else:
...
def fcn_np(par):
    ...
    centre, events_eN = fnc_events_MHVE(pp, ind, par[7],par[8])
...
np.savetxt('/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/SeN_chisq/chisq_ML_N.txt', np.c_[chisq_ML,chisqndf])
np.savetxt('/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/SeN_chisq/MLvalues_N.txt', np.c_[m.values])

txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/SeN_chisq/MHVE_Fef_sN_mparams.txt", "w")
