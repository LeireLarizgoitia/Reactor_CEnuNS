...
def fnc_events_MHVE_e(qe=1., qnu=1., gg=0.,mm=0., parsys=[1.]):
    ...
    Ei = np.linspace(Eee_thres, T_max, num=1000, endpoint=True, dtype=float)
    Enr = Ei
    for x in Ei:
...
np.savetxt('/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/SeN_chisq/chisq_ML_y_e.txt', np.c_[chisq_ML,chisqndf])
np.savetxt('/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/SeN_chisq/MLvalues_y_e.txt', np.c_[m.values])

txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/SeN_chisq/MHVE_YBe_se_mparams.txt", "w")
