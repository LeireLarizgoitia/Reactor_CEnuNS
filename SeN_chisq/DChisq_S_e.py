...
cSMe , eSMe = np.loadtxt("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Counts_SMe.txt",unpack=True) #/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/
cSMN , eSMN_Fef, eSMN_YBe = np.loadtxt("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Counts_SMN_Fef_YBe.txt",unpack=True)


def em_interp(mi):
    ml, cml, eml = np.loadtxt("Events_se_m_tot.txt",unpack=True)
    y = []
    for j in range(0,130):
        mm = []
        em = []
        for i in range(0,len(ml),130):
            mm.append(ml[i+j])
            em.append(eml[i+j])
        if mi<=mm[len(mm)-1] and mi>=mm[0]:
            xnew = [mi]
            ym = np.interp(xnew,mm,em)
        y.append(ym[0])
    return y
...
def fcn_np(par):
    ...
    ese_m = em_interp(par[8]) #mass parameter

    events=[]
    for i in range(0,len(cSMe)):
        events.append(eSMN_Fef[i] + eSMe[i] + ese_m[i]*(par[7])**4) #"Leptonic - Fef"
...
chisq_ML = 1.038293054485773865e+02 #fcn_np(m.values)


'mu. Delta chi2 analysis'
g_low_scan = 1e-7
g_up_scan = 1e-4
m_low_scan = 1.000000000000000194e-01
m_up_scan = 1e6
g_chi=[]
m_chi=[]
m_list=[]
...
for i in range(0,nscan): #range of scan
    m_chi.append(m_low_scan + (m_up_scan - m_low_scan)/nscan * i)
    for j in range(0,nscan): #range of scan
        g_chi.append(g_low_scan + (g_up_scan - g_low_scan)/nscan * j)
        m_list.append(m_chi[i])
        print(m_chi[i],g_chi[j])

        fcn_np.errordef = 1.

        m = Minuit(fcn_np, (100, 1.297, 0.1, 20., 150, 4, 1.,g_chi[j],m_chi[i]) ,
        ...
        m.fixed['a_g'] = True  # analysis for mu so each time a_mu must be fixed
        m.fixed['a_m'] = True  # analysis for mu so each time a_mu must be fixed

        resum = m.migrad() #.simplex()#

        #resume_minos = m.minos(cl = 0.9)

        deltachisq.append(fcn_np(m.values) - chisq_ML)

        print(m_chi[i],g_chi[j],deltachisq[j])
    ...
    #np.savetxt("MHVE_Fef_mue_Deltachi2mu_"+str(i)+".txt", np.c_[mu_chi,deltachisq])

    #print(m.params)

np.savetxt('/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/SeN_chisq/Dchi2_se_Fef.txt', np.c_[m_list,g_chi,deltachisq])
...
m = Minuit(fcn_np, (100, 1.297, 0.1, 20., 150, 4, 1.,1e-5,1e0) ,
           name=('hL1', 'cL1', 'wL1', 'An', 'Bn', 'Cn', 'aM_prior','a_g','a_m')) #,'a_norm','a_res','a_QF')) #start_1,start_1,start_1)
...
np.savetxt('chisq_ML_e.txt', np.c_[chisq_ML,chisqndf])
np.savetxt('MLvalues_e.txt', np.c_[m.values])

txt_file = open("MHVE_Fef_se_mparams.txt", "w")
...
np.savetxt('/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/SeN_chisq/MLvalues_e.txt', np.c_[m.values])

txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/SeN_chisq/MHVE_Fef_se_mparams.txt", "w")
content = str(m.params)
txt_file.write("".join(content) + "\n")
txt_file.close()
"""
