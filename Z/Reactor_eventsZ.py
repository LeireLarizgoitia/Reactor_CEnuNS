...
def fnc_events_MHVE_Fef(parsys=[1.,1.],gz2=0., mz2=0.):
    Eee_QF = []
    dNdxQF = []
    ...
    for i in range(0,nsteps+1):
        ...
        Eee_QF.append(x)
        dNdxQF.append( normalization *(differential_events_flux_Z(t,gz2, mz2)) * 1 / QF(t, parsys[1]) * (1- x/QF(t, parsys[1]) * dQdEI(x, parsys[1])))
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Z/Counts_MVHE_Fef_ZSM.txt", "w")
...
m = Minuit(fcn_np, (100, 1.297, 0.1, 20., 150, 4, 1.,0.,0.) ,
           name=('hL1', 'cL1', 'wL1', 'An', 'Bn', 'Cn', 'aM_prior','a_gz2','a_mz2')) #,'a_norm','a_res','a_QF')) #start_1,start_1,start_1)
...
m.limits['a_gz2'] = (0.0,1.)
m.limits['a_mz2'] = (0.0,None)
...
centre_Z, events_Z= fnc_events_MHVE_Fef([1.,1.],agz2_ML,amz2_ML)
...
fitcurve_ON = fnc_fitON(E_ion, parL1_ML, aM_prior_ML, parexp_ML, wM_ML,  events_Z)

#txt_file = open("/scratch/franmon/Leire/Reactor_CEnuNS/SM/MHVE_Fef_SM_mparams90cl.txt", "w")
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Z/MHVE_Fef_Z_mparams90cl.txt", "w")
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Z/MHVE_Fef_Z_chisq90cl.txt", "w")
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Z/MHVE_Fef_Z_fit90cl.txt", "w")
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Z/MHVE_Fef_Z_events90cl.txt", "w")
for aa in range(0,len(E_ion)):
    a_obs = E_ion[aa]
    e_obs = events_Z[aa]
    content = str(a_obs)+' '+ str(e_obs)
