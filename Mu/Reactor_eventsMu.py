...
def fnc_events_MHVE_Fef(parsys=[1.,1.],mu2=0.):
    Eee_QF = []
    dNdxQF = []
    ...
    for i in range(0,nsteps+1):
        ...
        Eee_QF.append(x)
        dNdxQF.append( normalization *(differential_events_flux_mu(t,mu2)) * 1 / QF(t, parsys[1]) * (1- x/QF(t, parsys[1]) * dQdEI(x, parsys[1])))
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Mu/MHVE_Fef_Mu_Deltachi2.txt", "w")
