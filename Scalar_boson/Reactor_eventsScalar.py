...
def fnc_events_MHVE_Fef(parsys=[1.,1.],gphi2=0.,mphi2=0.):
    Eee_QF = []
    dNdxQF = []
    ...
    for i in range(0,nsteps+1):
        ...
        Eee_QF.append(x)
        dNdxQF.append( normalization *(differential_events_flux_scalar(t,gphi2,mphi2)) * 1 / QF(t, parsys[1]) * (1- x/QF(t, parsys[1]) * dQdEI(x, parsys[1])))
...
centre, events_MHVE_Fef = fnc_events_MHVE_Fef([1.,1.],0.,0.)
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Scalar_boson/Counts_MVHE_Fef_PhiSM.txt", "w")
...
def fcn_np(par):
    ...
    centre, events_NOQw = fnc_events_MHVE_Fef(par_sys,par_phi2[0],par_phi2[1])
...
m = Minuit(fcn_np, (100, 1.297, 0.1, 20., 150, 4, 1.,0.,0.) ,
           name=('hL1', 'cL1', 'wL1', 'An', 'Bn', 'Cn', 'aM_prior','a_gphi2','a_mphi2')) #,'a_norm','a_res','a_QF')) #start_1,start_1,start_1)
...
m.limits['a_gphi2'] = (0.0,1.)
m.limits['a_mphi2'] = (0.0,None)
...
agphi2_ML = m.values[7]
amphi2_ML = m.values[8]
...
agphi2_err = m.errors[7]
amphi2_err = m.errors[8]
...
centre_phi, events_phi= fnc_events_MHVE_Fef([1.,1.],agphi2_ML,amphi2_ML)
...
fitcurve_ON = fnc_fitON(E_ion, parL1_ML, aM_prior_ML, parexp_ML, wM_ML,  events_phi)

#txt_file = open("/scratch/franmon/Leire/Reactor_CEnuNS/SM/MHVE_Fef_SM_mparams90cl.txt", "w")
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Scalar_boson/MHVE_Fef_Phi_mparams90cl.txt", "w")
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Scalar_boson/MHVE_Fef_Phi_chisq90cl.txt", "w")
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Scalar_boson/MHVE_Fef_Phi_fit90cl.txt", "w")
...
txt_file = open("/scratch/llarizgoitia/Reactor/Reactor_CEnuNS/Scalar_boson/MHVE_Fef_Phi_events90cl.txt", "w")
for aa in range(0,len(E_ion)):
    a_obs = E_ion[aa]
    e_obs = events_phi[aa]
    content = str(a_obs)+' '+ str(e_obs)
